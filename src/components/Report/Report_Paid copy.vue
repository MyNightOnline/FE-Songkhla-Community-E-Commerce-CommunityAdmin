      ...
      <div class="flex items-center ">
        <div class="w-64">
          <VueDatePicker locale="th" lang="th" :format="test" :format-locale="th" position="left" v-model="date" range
            ...
            selectText="บันทึก" @update:model-value="test2($event)">
            <template #year="{ year }">
              {{ year + 543 }}
            </template>
...
onMounted(async () => {
  ...
  date.value = [response.data[lenReponse - 1].date, response.data[0].date]
})
  ...
  methods: {
    ...
    test2(v: any) {
      ...
      v.forEach((item: any, index: number) => {
        ...
        if (index == 0) date1 = `${dayOfMonth}-${month}-${year}`
        else if (index == 1) date2 = `${dayOfMonth}-${month}-${year}`
      })
      ...
      this.orders = this.defaultOrders.filter((order: any) => {
        ...
        if (fullDate >= date1 && fullDate <= date2) {
